...
class RepetitionEstimator():
  ...
  def __init__(self, num_outputs, reuse=False, trainable=True):
    ...
    self.targets = tf.placeholder(shape=[None], dtype=tf.float32, name="y")
    # Integer id of which action was selected
    self.repetitions = tf.placeholder(shape=[None], dtype=tf.int32, name="repetitions")
    ...
    with tf.variable_scope("repetition_net"):
      self.logits = tf.contrib.layers.fully_connected(fc1, num_outputs, activation_fn=None)
      self.probs = tf.nn.softmax(self.logits) + 1e-8
      ...
      # Get the predictions for the chosen actions only
      gather_indices = tf.range(batch_size) * tf.shape(self.probs)[1] + self.repetitions
      self.picked_repetition_probs = tf.gather(tf.reshape(self.probs, [-1]), gather_indices)

      self.losses = - (tf.log(self.picked_repetition_probs) * self.targets + 0.01 * self.entropy)
      self.loss = tf.reduce_sum(self.losses, name="loss")

      tf.summary.scalar(self.loss.op.name, self.loss)
      tf.summary.scalar(self.entropy_mean.op.name, self.entropy_mean)
      tf.summary.histogram(self.entropy.op.name, self.entropy)
      # tf.summary.scalar(self.repetitions.name,self.repetitions[num_rep])
      tf.summary.histogram(self.repetitions.name,self.repetitions)
